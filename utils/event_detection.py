...
class EventDetection():
   ...
   def check_ball_out(self,frame,  classes, boxes, LeftUp, LeftDown, RightDown, RightUp):
      ...
      if self.current_ball is None or self.prev_ball is None or self.pre_prev_ball is None: return None
      
      cv2.line(frame, (self.current_ball[0], self.current_ball[1]), (self.pre_prev_ball[0], self.pre_prev_ball[1]), (0,255,0), 4, cv2.LINE_AA)
      ...
      # Change direction
      if self.current_slop != self.prev_slop:
         ...
         if BallInOut >= 0:
            ...
            # Ball Out
            cv2.putText(frame, "BALL OUT :" + str(abs(BallInOut)) + "cm", (25, 25), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255), 3)
            return True
   ...
   def claculate_ball_slope(self, pointA, pointB):
      if (pointA[0] - pointB[0]) == 0 : return 0
